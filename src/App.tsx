...
export default function App() {
  ...
  useEffect(() => {
    const cachedCart = localStorage.getItem('hh_cart');
    if (cachedCart) {
      try {
    ...
    const cachedBookings = localStorage.getItem('hh_bookings');
    if (cachedBookings) {
      try {
  ...
  const saveCart = (newCart: CartItem[]) => {
    setCart(newCart);
    localStorage.setItem('hh_cart', JSON.stringify(newCart));
  };
  ...
  const handleAddBooking = (newBooking: TableBooking) => {
    ...
    localStorage.setItem('hh_bookings', JSON.stringify(updated));
  };
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-brand-outline leading-relaxed font-sans max-w-xs">
              © 2024 hh mumbai. Handcrafted using eco-conscious Mumbai local clay and slow artisanal materials.
            </p>
